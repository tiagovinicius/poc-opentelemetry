...
import { BinaryCarrier, FORMAT_BINARY, FORMAT_TEXT_MAP, Reference, REFERENCE_CHILD_OF, Span, Tracer } from 'opentracing';
 ...
 */
function apiCompatibilityChecks(
  createTracer = () => new Tracer(),
  options: ApiCompatibilityChecksOptions = {
    skipBaggageChecks: false,
    skipInjectExtractChecks: false },
): void {
    ...
    describe('Tracer', () => {
      describe('startSpan', () => {
        it('should handle Spans and SpanContexts', () => {
          expect(() => { tracer.startSpan('child', { childOf: span }); })
            .to.not.throw(Error);
          expect(() => { tracer.startSpan('child', { childOf: span.context() }); })
            .to.not.throw(Error);
        });
      });
      ...
      describe('inject', () => {
        (options.skipInjectExtractChecks ? it.skip : it)('should not throw exception on required carrier types', () => {
          ...
          expect(() => { tracer.inject(spanContext, FORMAT_TEXT_MAP, textCarrier); })
            .to.not.throw(Error);
          expect(() => { tracer.inject(spanContext, FORMAT_BINARY, binCarrier); })
            .to.not.throw(Error);
          expect(() => { tracer.inject(spanContext, FORMAT_BINARY, {}); })
            .to.not.throw(Error);
        });

        (options.skipInjectExtractChecks ? it.skip : it)('should handle Spans and SpanContexts', () => {
          const textCarrier = {};
          expect(() => { tracer.inject(span, FORMAT_TEXT_MAP, textCarrier); })
            .to.not.throw(Error);
      ...
      describe('extract', () => {
        (options.skipInjectExtractChecks ? it.skip : it)('should not throw exception on required carrier types', () => {
          const textCarrier = {};
          const binCarrier = new BinaryCarrier([1, 2, 3]);
          expect(() => { tracer.extract(FORMAT_TEXT_MAP, textCarrier); })
            .to.not.throw(Error);
          expect(() => { tracer.extract(FORMAT_BINARY, binCarrier); })
            .to.not.throw(Error);
          expect(() => { tracer.extract(FORMAT_BINARY, {}); })
            .to.not.throw(Error);
